...
fn main() -> anyhow::Result<()> {
    ...
    eprintln!("=== GOOD prompt: last tok = {} ({:?}) ===", ids1[ids1.len()-1], ctx.tokenizer().decode(&[ids1[ids1.len()-1]]));
    let logits = ctx.forward(&[ids1[ids1.len()-1]])?;
    let mut indexed: Vec<(usize, f32)> = logits.iter().copied().enumerate().collect();
    indexed.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap());
    eprintln!("Top: {:?}({:.2})", ctx.tokenizer().decode(&[indexed[0].0 as u32]), indexed[0].1);
    ...
    std::env::set_var("POWERINFER_TRACE_TOKENS", "1");
    eprintln!("\n=== BAD prompt: last tok = {} ({:?}) ===", ids2[ids2.len()-1], ctx.tokenizer().decode(&[ids2[ids2.len()-1]]));
    ...
    std::env::set_var("POWERINFER_TRACE_TOKENS", "1");
    eprintln!("\n=== ENGLISH 19tok: last tok = {} ({:?}) ===", ids3[ids3.len()-1], ctx.tokenizer().decode(&[ids3[ids3.len()-1]]));
