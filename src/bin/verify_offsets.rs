...
fn main() -> anyhow::Result<()> {
    ...
    for t in gguf.tensors() {
        if t.name == "token_embd.weight" || t.name == "blk.0.attn_norm.weight" || t.name == "output.weight" {
            println!("  {} offset={} size={} shape={:?} kind={:?}", t.name, t.offset, t.size, t.shape, t.kind);
        }
    }
    ...
    println!("  token_embd.weight: shape={:?} qtype={:?}", embd.shape, embd.qtype);
    println!("  blk.0.attn_norm.weight: shape={:?} qtype={:?}", norm0.shape, norm0.qtype);
    ...
    println!("  bytes: {:02x} {:02x} {:02x} {:02x}", first4[0], first4[1], first4[2], first4[3]);
