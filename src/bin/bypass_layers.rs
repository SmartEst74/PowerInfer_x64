...
fn main() -> anyhow::Result<()> {
    ...
    powerinfer::quant::matvec_col_major(
        &mut logits, &normed, output_t.raw(), output_t.qtype, n_embd, n_vocab,
    )?;
    ...
    powerinfer::quant::matvec_col_major(
        &mut logits2, &normed, token_embd_t.raw(), token_embd_t.qtype, n_embd, n_vocab,
    )?;
    
    println!("TOKEN_EMBD_AS_LM_HEAD_TOP10");
    let mut indexed2: Vec<(usize, f32)> = logits2.iter().enumerate().map(|(i, &v)| (i, v)).collect();
